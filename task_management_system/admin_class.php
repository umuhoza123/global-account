...
class Action
{
	...
	function approve_user()
	{
		...
		if (!empty($id)) {
			$update_users = $this->db->query("INSERT INTO users set $data");
			if ($update_users) {
				$update_system = $this->db->query("UPDATE system_newclient set status='APPROVE' where id = $id");  // Update system_newclient status
			} // Use UPDATE query for users
		...
		if ($save || ($update_users && $update_system)) {
			return 1;
		} else {
	...
	function save_device()
	{
		...
		// echo $data;exit;
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO system_device set $data");
		} else {
			$save = $this->db->query("UPDATE system_device set $data where id = $id");
		}
		if ($save) {
			return 1;
		}
	...
	function save_branch()
	{
		...
		// echo $data;exit;
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO system_branch set $data");
		} else {
			$save = $this->db->query("UPDATE system_branch set $data where id = $id");
		}
		if ($save) {
			return 1;
		}
	...
	function save_staff()
	{
		...
		// echo $data;exit;
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO system_staff set $data");
		} else {
			$save = $this->db->query("UPDATE system_staff set $data where id = $id");
		}
		if ($save) {
			return 1;
		}
